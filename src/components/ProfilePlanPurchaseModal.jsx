...
const ProfilePlanPurchaseModal = ({ onClose }) => {
  ...
  const pleaseCompletePaymentText = useTranslatedText("Please complete the payment authentication if prompted.");
  ...
  const [selectedProfile, setSelectedProfile] = useState(newProfile[0]); // Default to 1 Profile
    ...
    try {
      if (paymentList?.defaultPaymentMethodId) {
        ...
        if (data.success && !data.requiresAction) {
          ...
          onClose();
          // Refresh the page to update profile status in the UI
          window.location.reload();
        } else if (data.success && data.requiresAction && data.client_secret) {
          console.log("Payment requires action...");
          // In a real app, you would typically render Stripe Elements here for 3D Secure
          // For now, we'll just show a message and potentially navigate
          message.info(pleaseCompletePaymentText);
          onClose();
          // You might navigate to a dedicated payment completion/authentication page
          // navigate('/payment/confirm', { state: { clientSecret: data.client_secret } });

        } else {
          // Handle cases like currentPlan message or other API errors
  ...
  // Function to handle card selection (optional, as slider already selects)
      ...
      <ModalBody className="bg-[#1f1634] ">
        <div className="flex justify-center">
          <Box className="text-center ms-4 w-fit relative top-1 text-xl md:text-2xl lg:text-2xl text-[#f3e4e4] font-[verdana]">
            ...
            {/* Removed maxFriend related text as this modal is for purchasing a new plan */}
          </Box>
        </div>
        ...
        {/* Continue button and card section - always show in this purchase modal */}
        <div className="w-full mt-8 flex flex-col items-center justify-center">
          {/* Card content based on selectedProfile (from slider) */}
          <div
            className={`
                flex w-full cursor-pointer rounded-[14px] overflow-hidden
                border-2 border-[#e48af7] \n                mt-6 mb-6
              `}
          // onClick={() => handleClick(newProfile[sliderValue])} // Click handled by slider
          >
...
export default ProfilePlanPurchaseModal; 